...
from loss import compute_pg_loss, compute_Q_loss
from utils import (
    find_free_port,
...
SCRATCH = Path("/user/work/dg22309/grpo/QPO")
...
def main():
    ...
    parser.add_argument(
        "--algo",
        type=str,
        choices=["grpo", "dr_grpo", "dapo", "optimal", "QPO"],
        default="QPO",
        help="Algorithm variant: grpo (original), dr_grpo (unbiased), dapo (state-of-the-art), optimal (best combined configuration), or QPO (Q-learning)",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="countdown",
        help="Dataset to use for training (e.g., 'countdown', 'gsm8k'). See datasets.py for options.",
    )
    parser.add_argument(
        "--optimal",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="mc",
        choices=["mc", "online_bootstrap", "q_learning"],
        help="Training method: mc (Monte Carlo), online_bootstrap (online bootstrap), or q_learning (Q-learning)",
    )
    parser.add_argument(
        "--Q_beta",
        type=float,
        default=1.0,
        help="Beta for Q-learning",
    )
    parser.add_argument(
        "--Q_A",
        type=float,
    ...
    dataset = load_dataset(
        ...
        name=dataset_config.get("config_name"),
        split=dataset_config["split"],
    )
    dataset = dataset.map(
        dataset_config["preprocess_fnc"],
        num_proc=6,
    ...
    for iteration in trange(begin_iter, NUM_ITERATIONS):
        ...
        # TODO: add evaluation for iteration 0 back in (!!)
        eval_stats = None
